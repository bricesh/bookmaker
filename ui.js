...
export function openModal({eyebrow,title,body,saveLabel="Save",onSave,extraFoot}){
  ...
  const scrim = h(".scrim",{onclick:(e)=>{if(e.target===scrim)closeModal();}},
...
export function confirmDlg(title,msg,yes="Delete"){
  return new Promise(res=>{
    ...
    const root=document.getElementById("modal-root");
    root.querySelector(".scrim").addEventListener("click",e=>{if(e.target.classList.contains("scrim"))res(false);});
